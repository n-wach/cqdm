"""
Module version for monitoring CLI pipes (`... | python -m tqdm_base | ...`).
"""
import logging
import re
import sys
...
def posix_pipe(fin, fout, delim=b'\\n', buf_size=256,
    ...
    callback  : function(float), e.g.: `tqdm_base.update`
...
CLI_EXTRA_DOC = r"""
        ...
        manpath  : str, optional
            Directory in which to install tqdm_base man pages.
        comppath  : str, optional
            Directory in which to place tqdm_base completion.
        log  : str, optional
...
def main(fp=sys.stderr, argv=None):
    ...
    ---------
    fp  : file-like object for tqdm_base
...
""" + d.strip('\n') + '\n'
    ...
    argv = RE_SHLEX.split(' '.join(["tqdm_base"] + argv))
        ...
        if manpath or comppath:
            ...
            def cp(name, dst):
                ...
                if hasattr(resources, 'files'):
                    copyfile(str(resources.files('tqdm_base') / name), dst)
                else:  # py<3.9
                    with resources.path('tqdm_base', name) as src:
                        copyfile(str(src), dst)
                log.info("written:%s", dst)
            if manpath is not None:
                cp('tqdm_base.1', path.join(manpath, 'tqdm.1'))
