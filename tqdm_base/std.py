...
Usage:
>>> from tqdm_base import trange, tqdm
>>> for i in trange(10):
...     ...
"""
...
class TqdmWarning(Warning):
    """base class for all tqdm_base warnings.
...
class TqdmMonitorWarning(TqdmWarning, RuntimeWarning):
    """tqdm_base monitor errors which do not affect external functionality"""
    pass
...
class TqdmDefaultWriteLock(object):
    ...
    You must initialise a `tqdm_base` or `TqdmDefaultWriteLock` instance
...
class tqdm(Comparable):
        ...
        with cls.get_lock():  # also constructs lock if non-existent
            ...
            if cls.monitor_interval and (cls.monitor is None
                ...
                try:
                    ...
                    warn("tqdm_base:disabling monitor support"
    ...
    def _decr_instances(cls, instance):
        ...
        (tqdm_base<=4.44.1 moved ALL subsequent unfixed bars up.)
        """
        with cls._lock:
            try:
    ...
    def write(cls, s, file=None, end="\n", nolock=False):
        """Print a message via tqdm_base (without overlap with bars)."""
    ...
    def external_write_mode(cls, file=None, nolock=False):
        """
        Disable tqdm_base within context and refresh tqdm_base when exits.
        ...
        try:
            ...
            for inst in getattr(cls, '_instances', []):
                ...
                # or if write output + tqdm_base output are both either
    ...
    def pandas(cls, **tqdm_kwargs):
        ...
        >>> import pandas as pd
        >>> import numpy as np
        >>> from tqdm_base import tqdm
        >>> from tqdm_base.gui import tqdm as tqdm_gui
        >>>
        ...
        # Enable custom tqdm_base progress in pandas!
    ...
    def __init__(self, iterable=None, desc=None, total=None, leave=True, file=None,
        ...
        gui  : bool, optional
            ...
            Use tqdm_base.gui.tqdm_base(...) instead. If set, will attempt to use
    ...
    def __reversed__(self):
        try:
            ...
            raise TypeError("'tqdm_base' object is not reversible")
        else:
    ...
    def __iter__(self):
        """Backward-compatibility to use: for x in tqdm_base(iterable)"""
        yield from tqdmc.tqdmciter(self)
    ...
    def update(self, n=1):
        ...
        E.g.:
        >>> t = tqdm_base(total=filesize) # Initialise
        ...
        if self.n - self.last_print_n >= self.miniters:
            ...
            if dt >= self.mininterval and cur_t >= self.start_t + self.delay:
                ...
                if self.dynamic_miniters:
                    ...
                    # e.g.: After running `tqdm_base.update(5)`, subsequent
                    # calls to `tqdm_base.update()` will only cause an update after
    ...
    def unpause(self):
        """Restart tqdm_base timer from last print time."""
        if self.disable:
            return
        cur_t = self._time()
    ...
    def display(self, msg=None, pos=None):
        ...
        if not hasattr(self, "sp"):
            raise TqdmDeprecationWarning(
                "Please use `tqdm_base.gui.tqdm_base(...)`"
                " instead of `tqdm_base(..., gui=True)`\n",
    ...
    def wrapattr(cls, stream, method, total=None, bytes=True, **tqdm_kwargs):
        ...
        >>> with tqdm_base.wrapattr(file_obj, "read", total=file_obj.size) as fobj:
        ...     while True:
...
def trange(*args, **kwargs):
    """
    A shortcut for tqdm_base(xrange(*args), **kwargs).
